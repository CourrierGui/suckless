...
static int vp;               /* vertical padding of the bar */
...
void pushstack(const Arg *arg) { int i = stackpos(arg);
...
int stackpos(const Arg *arg) { int n, i;
  Client *c, *l;
...
void updatebars(void) {
  ...
  for (m = mons; m; m = m->next) {
    ...
    m->barwin = XCreateWindow(
      dpy, root, m->wx+sp, m->by+vp, m->ww - 2*sp, bh, 0, depth,
      InputOutput, visual,
...
void view(const Arg *arg) {
  // if the selected tag is the same as the current one, stop.
